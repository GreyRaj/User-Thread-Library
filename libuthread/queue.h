 ...
 * the new queue.
 */
queue_t queue_create(void);		// question
 ...
 */
int queue_destroy(queue_t queue);		// question
 ...
 */
int queue_enqueue(queue_t queue, void *data);		// question
 ...
 */
int queue_dequeue(queue_t queue, void **data);		// question
 ...
 */
int queue_iterate(queue_t queue, queue_func_t func);		// question
...
#define qMAXlen 30
// question is it ok to have this here:
struct node {
	void* data;
	struct node* next;
};
...
// QUESTION: also see attempt at queue_delete in main() ~~~ not working
